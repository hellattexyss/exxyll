...
function Camlock:Toggle()
    local currentTime = tick()
    if currentTime - self.LastToggleTime < self.Cooldown or self.Locked then
        return self.Enabled
    end
...
function Camlock:CreateMobileButton()
    ...
    local inputChangedConn = game:GetService("UserInputService").InputChanged:Connect(function(input)
        if input == dragInput and dragging then
            updateDrag(input)
        end
    end)
    
    -- Store connection in input connections
    table.insert(self.InputConnections, inputChangedConn)
...
-- Snippet 4/4: UI Setup and Final Initialization
    ...
    -- Fixed keybind handler for camlock
