...
-- Snippet 3/5: Camlock System with Fixed Mobile Button
local Camlock = {
    Enabled = ConfigManager:Get("CamlockEnabled"),  -- Load saved state
    Target = nil,
    TargetHighlight = nil,
    Connections = {},
    MobileButton = nil,
    ...
    LastClickTime = 0,
    ClickCooldown = 0.3,
    Prediction = ConfigManager:Get("CamlockPrediction") or 0.5,  -- Load saved prediction
    ShowNotifications = ConfigManager:Get("CamlockNotificationsEnabled"),  -- Load saved setting
    ShowTargetInfo = ConfigManager:Get("CamlockTargetInfoEnabled"),  -- Load saved setting
    Keybind = ConfigManager:Get("CamlockKeybind") or "Q",  -- Load saved keybind
    MobileButtonVisible = ConfigManager:Get("MobileCamlockButton") or false,  -- Load saved mobile button state
    IsDragging = false,
    DragStartPos = nil,
    ButtonStartPos = nil,
    ButtonHasPositionLock = false,
    SavedButtonPosition = nil
}

-- Load saved button position
if isfile("CamlockButtonPosition.json") then
    local success, data = pcall(function()
        return game:GetService("HttpService"):JSONDecode(readfile("CamlockButtonPosition.json"))
    end)
    if success and data then
        Camlock.SavedButtonPosition = UDim2.new(0, data.X or 0, 0, data.Y or 0)
    end
end
...
function Camlock:Start()
    ...
    ConfigManager:Set("CamlockEnabled", true)  -- Save state
...
function Camlock:Stop()
    ...
    ConfigManager:Set("CamlockEnabled", false)  -- Save state
...
function Camlock:SaveButtonPosition(position)
    local success = pcall(function()
        local data = {
            X = position.X.Offset,
            Y = position.Y.Offset
        }
        writefile("CamlockButtonPosition.json", game:GetService("HttpService"):JSONEncode(data))
    end)
    return success
end
...
function Camlock:CreateMobileButton()
    ...
    if self.SavedButtonPosition then
        button.Position = self.SavedButtonPosition
    else
        -- Default position (right side, centered vertically)
    ...
    button.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseButton1 then
            -- First touch: capture current position
            isDragging = true
            dragStartPosition = Vector2.new(input.Position.X, input.Position.Y)
            buttonStartPosition = Vector2.new(button.Position.X.Offset, button.Position.Y.Offset)
    ...
    button.InputChanged:Connect(function(input)
        if isDragging and (input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseMovement) then
            ...
            local newX = math.clamp(
                buttonStartPosition.X + delta.X,
                0,
                viewportSize.X - button.AbsoluteSize.X
            )
            
            local newY = math.clamp(
                buttonStartPosition.Y + delta.Y,
                0,
    ...
    button.InputEnded:Connect(function(input)
        if isDragging and (input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseButton1) then
            ...
            self:SaveButtonPosition(button.Position)
            ...
            local moveDistance = (endPosition - dragStartPosition).Magnitude
            
            if moveDistance < 10 then -- Click threshold
                self:Toggle()
            end
        end
    end)
...
function Camlock:UpdateKeybind(newKey)
    self.Keybind = newKey
    ConfigManager:Set("CamlockKeybind", newKey)
    self:SetupKeybind()
end
...
function Camlock:SetupKeybind()
    ...
    self.KeybindConnection = self.InputService.InputBegan:Connect(function(input, processed)
        if processed or not input.KeyCode then
            return
        end
...
-- Initialize button state based on saved state
...
local camlockToggle = CamlockTab:Toggle({
    ...
    Value = Camlock.Enabled,  -- Use actual Camlock state
    Callback = function(state)
        if state then
...
-- FIXED: Replace button with Input field for keybind
local keybindInput = CamlockTab:Input({
    Title = "Camlock Keybind",
    Desc = "Set the key to toggle camlock",
    Value = Camlock.Keybind,
    InputIcon = "key",
    Type = "Input",
    Placeholder = "Press a key...",
    Callback = function(input)
        if input and #input > 0 then
            local keyName = input:upper()
            Camlock:UpdateKeybind(keyName)
            WindUI:Notify({
                Title = "Keybind Updated",
                Content = "Camlock keybind set to: " .. keyName,
                Duration = 3,
                Icon = "check"
            })
        end
...
local mobileCamlockToggle = CamlockTab:Toggle({
    ...
    Desc = "Show mobile button for camlock",
    Value = Camlock.MobileButtonVisible,
    Callback = function(state)
        ConfigManager:Set("MobileCamlockButton", state)
        Camlock.MobileButtonVisible = state
...
-- REMOVED: Camlock Notifications Toggle (as per requirements)
-- All notifications except keybind changes are removed
...
-- Create mobile button if enabled
if Camlock.MobileButtonVisible then
    Camlock:CreateMobileButton()
end
