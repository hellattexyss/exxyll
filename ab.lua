...
function Camlock:Toggle()
    ...
    if self.Enabled then
        ...
        -- Turn ON - only if config allows it
        if not ConfigManager:Get("CamlockEnabled") then
            return false
        end
...
function Camlock:CreateMobileButton()
    ...
    -- Store drag connection in input connections
    ...
    button.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            -- Only start dragging if not clicking the button itself
            if input.Parent == button then
                dragging = true
    ...
    local function handleCamlockToggle()
        ...
        if currentTime - lastClickTime > clickCooldown then
            ...
            -- Check if camlock is actually enabled in config
            if ConfigManager:Get("CamlockEnabled") then
                self:Toggle()
            else
                WindUI:Notify({
                    Title = "Camlock",
                    Content = "Enable Camlock in settings first!",
                    Duration = 2,
                    Icon = "alert-triangle"
                })
            end
    ...
    keybindConnection = game:GetService("UserInputService").InputBegan:Connect(function(input, processed)
        ...
        if not processed and input.KeyCode.Name == ConfigManager:Get("CamlockKeybind") then
            ...
            if currentTime - lastKeybindTime > keybindCooldown then
                ...
                -- Check if camlock is enabled in config
                if not ConfigManager:Get("CamlockEnabled") then
                    WindUI:Notify({
                        Title = "Camlock",
                        Content = "Enable Camlock in settings first!",
